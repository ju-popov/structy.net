...
func TestBreadthFirst(t *testing.T) {
	...
	for _, testCase := range testCases {
		...
		t.Run(testCase.name, func(t *testing.T) {
			t.Parallel()
			actual := undirectedpath.BreadthFirst(testCase.edges, testCase.nodeA, testCase.nodeB)
...
func benchmarkBreadthFirst(b *testing.B, testCase testCase) {
	...
	for n := 0; n < b.N; n++ {
		undirectedpath.BreadthFirst(testCase.edges, testCase.nodeA, testCase.nodeB)
	}
}
...
func BenchmarkBreadthFirst008(b *testing.B) { benchmarkBreadthFirst(b, testCases[8]) }
